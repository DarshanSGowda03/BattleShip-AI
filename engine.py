...
# search in neighborhood of hits
        unknown_with_neighbour_hits1 = []
        unknown_with_neighbour_hits2 = []
        for u in unknown:
            if u+1 in hits or u-1 in hits or u-10 in hits or u+10 in hits:
                unknown_with_neighbour_hits1.append(u)
            if u+2 in hits or u-2 in hits or u-20 in hits or u+20 in hits:
                unknown_with_neighbour_hits2.append(u)

        # pick "U" Square with direct and level-2 neighbour both marked as "H"
        for u in unknown:
            if u in unknown_with_neighbour_hits1 and u in unknown_with_neighbour_hits2:
                self.make_move(u)
                return

        # pick "U" square that has a level - 1 neighbour marked as "H"
        if len(unknown_with_neighbour_hits1) > 0:
            self.make_move(random.choice(unknown_with_neighbour_hits1))
            return
